...
char* MD5hex(const void* data, int len, char* hexout)
{
  static const char* hexDigits = "0123456789abcdef";  // whiteboard server expects lowercase(!)

  uint8_t digest[MD5_DIGEST_SIZE];
  MD5(data, len > 0 ? len : strlen((const char*)data), digest);
  for(size_t ii = 0; ii < MD5_DIGEST_SIZE; ++ii) {
    hexout[2*ii] = hexDigits[(digest[ii] & 0xF0) >> 4];
    hexout[2*ii+1] = hexDigits[(digest[ii] & 0x0F)];
  }
  hexout[2*MD5_DIGEST_SIZE] = '\0';
  return hexout;
}
#endif
